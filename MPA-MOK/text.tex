...
Matice $\textbf{W}_{n \times n}$ je špatná báze a~tedy Alicin veřejný klíč.
...
Jde o~schéma obecně bezpečné, ale kvůli velikosti bezpečnostních parametrů nepraktické\footnotemark{}.
\footnotetext{
    Hoffstein, Pipher, Silverman (2014): An Introduction to Mathematical Cryptography.
    \\
...
Poté $b = \lfloor \frac{p}{q} \textbf{A}s \rceil$.

Pro~$p < q$ rozdělíme prvky $\mathbb{Z}_q$ do~$p$ sousedících intervalů (každých velkých zhruba $q/p$), a definujeme funkci zaokrouhlení $\lfloor \cdot \rceil_p$: $\mathbb{Z}_q \rightarrow \mathbb{Z}_p$ která mapuje $x \in \mathbb{Z}_q$ na~index intervalu do~kterého $x$ patří.
...
Saber je KEM schéma založené na~M-LWE.
